  ...
  filteredProducts.forEach((product) => {
    ...
    const productImages =
      product.images && product.images.length > 0
        ? product.images
        : ["https://via.placeholder.com/400x400/1a1a1a/22c55e?text=NEW"];
    ...
    card.innerHTML = `
      <div class="nr-image-container">
        ...
        <!-- SLIDER HTML -->
        <div class="product-slider" id="slider-nr-${product.id}">
            ${productImages
              .map(
                (img, i) => `
        ...
        ${
          productImages.length > 1
            ? `
            <button class="slider-nav slider-prev" onclick="slideNrImage(${product.id}, -1); event.stopPropagation();">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"></polyline></svg>
            </button>
            <button class="slider-nav slider-next" onclick="slideNrImage(${product.id}, 1); event.stopPropagation();">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"></polyline></svg>
            </button>
            <div class="slider-dots">
                ${productImages.map((_, i) => `<span class="slider-dot ${i === 0 ? "active" : ""}" onclick="goToNrSlide(${product.id}, ${i})"></span>`).join("")}
            </div>
        `
            : ""
        }
...
// ==================== LOGIKA SLIDER NEW RELEASE ====================
const nrSliderStates = {};

function slideNrImage(productId, direction) {
  const product = newReleaseProducts.find((p) => p.id === productId);
  if (!product || !product.images) return;

  const totalImages = product.images.length;
  if (!nrSliderStates[productId]) nrSliderStates[productId] = 0;

  nrSliderStates[productId] += direction;
  if (nrSliderStates[productId] < 0)
    nrSliderStates[productId] = totalImages - 1;
  if (nrSliderStates[productId] >= totalImages) nrSliderStates[productId] = 0;

  updateNrSlider(productId);
}

function goToNrSlide(productId, index) {
  nrSliderStates[productId] = index;
  updateNrSlider(productId);
}

function updateNrSlider(productId) {
  const slider = document.getElementById(`slider-nr-${productId}`);
  if (!slider) return;

  const dots = slider.parentElement.querySelectorAll(".slider-dot");
  slider.style.transform = `translateX(-${nrSliderStates[productId] * 100}%)`;

  dots.forEach((dot, i) => {
    dot.classList.toggle("active", i === nrSliderStates[productId]);
  });
}
